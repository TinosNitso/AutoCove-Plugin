...
Next version will have drag & drop for .txn & CashScript .artifact files, `<dec>` input, more examples, and lighter-on-black colors (need similar clarity on B&W backgrounds). There's a bugfix for when selecting asm switches to 'New', & no more exact duplicates in the memory. There should also be `dec` conversion (decoding to decimal). CashAddr toggle will be connected, and a '1 line' button to condense Scripts to 1 line.
