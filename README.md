...
This won't count as official "Command-line Integration" because `daemon_command` is only used to remote-control the GUI, which controls all settings (black, colors etc). Maybe there'll be interest in a purely web interface, but the server needs to run a GUI EC wallet (e.g. watching-only wallet). There'll also be ±Δ for the OpCodes List.
