...
Codes['BCH']='Cat Split Num2Bin Bin2Num And Or XOr Div Mod CheckDataSig CheckDataSigVerify' #'MS Shell Dlg 2' is default font but doesn't seem to allow adding serifs.
Codes['Disabled']='Invert 2Mul 2Div Mul LShift RShift'
if int(electroncash.version.PACKAGE_VERSION.replace('.',''))>=426: Codes['Native Introspection']='InputIndex ActiveBytecode TXVersion TXInputCount TXOutputCount TXLocktime  UTXOValue UTXOBytecode OutpointTXHash OutpointIndex InputBytecode InputSequenceNumber OutputValue OutputBytecode'    #Only color in Native Introspection OpCodes for EC-v4.2.6+.
...
ColorDict, HexDict, CodeDict = {}, {}, {}    #OpCode dictionaries. CodeDict is used by the decoder as a reversed HexDict. CodeDict also changes CODE to Code.
...
for key in Codes.keys()-{'BCH','Disabled'}:
    for Code in Codes[key].split():
        ...
        CodeDict[CodeUp]=Code #Convert upper to lower case.
...
class UI(QDialog):
    ...
    def __init__(self, window, plugin):
        ...
        self.CaseBox.setCurrentIndex(1), self.CaseBox.activated.connect(self.CaseSwitch)
        
        Title=QLabel('AutoCove v1.0.4-dev1')
        ...
        self.ScriptsBox.addItems(["New", "OpCodes", "v1.0.3"])
    ...
    def textChanged(self):  #Whenever users type, attempt to re-compile.
        ...
        Bytes=b''   #This section is the decoder. Start by checking if input is only 1 word & fully hex.
        if ' ' not in Script:
            try:    Bytes=bitcoin.bfh(Script)
            except: pass
        if Bytes:
            ...
            for Byte in Bytes:
                ...
                try:    #OpCode or else new data.
                    ...
                    Script+=CodeDict[OpCode]*(not Index) + 'OP_'*(Index>1) + OpCode*(Index>0)+' '
                    Words+=1
    ...
    def VersionSwitch(self):    #Change example assembly code with "correct" case.
        self.ScriptBox.setPlainText((self.Scripts[self.ScriptsBox.currentIndex()])), self.CaseSwitch()
    ...
    def CaseSwitch(self):   #Change btwn Codes, CODES & OP_CODES using QTextCursor. This is more complicated but possibly quicker than editing strings directly.
        Text, Cursor, Index = self.ScriptBox.toPlainText(), self.ScriptBox.textCursor(), self.CaseBox.currentIndex()
        ...
        for Line in Text.splitlines():
            ...
            for Word in LineCode.split():
                ...
                try:
                    insertWord = CodeDict[WordUp]*(not Index) + 'OP_'*(Index>1) + WordUp*(Index>0)
